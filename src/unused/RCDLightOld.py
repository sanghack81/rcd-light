...
# Three-phased RCD Light
class RCDLightOld(object):
    ...
    def applyColliderDetectionAndRBO(self, truth=None):
        ...
        for rv1, rv2, rv3, d12, d23 in sorted(self._findUnshieldedTriples(),
                                              key=lambda x: (
                                                          len(x[0].path) > self.hopThreshold + 1,
                                                          len(self.parents[x[2]]))):
...
def runRCDLightOld(schema, citest, hopThreshold, depth=None):
    rcd = RCDLightOld(schema, citest, hopThreshold, depth)
